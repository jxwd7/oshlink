/* ============================================================
   OSHLINK — Main JavaScript
   All 11 sections rendered and wired up
   ============================================================ */
...
// ---- Logo Ticker HTML ----
const logos = [
  'NovaTech', 'PeakScale', 'Meridian', 'Altura Group',
  'ClearPath', 'Orion Labs', 'Bluefin Co', 'Axiom Pro',
  'NovaTech', 'PeakScale', 'Meridian', 'Altura Group',
  'ClearPath', 'Orion Labs', 'Bluefin Co', 'Axiom Pro',
];
const tickerHTML = `
<section id="logo-ticker" aria-label="Trusted by">
  ...
  <div class="ticker-track" aria-hidden="true">
    <div class="ticker-inner">
      ${logos.map(l => `<div class="ticker-logo">${l}</div>`).join('')}
    </div>
  </div>
  <div class="container">
...
const services = [
  {
    front: 'Never Miss a Warm Lead Again',
    cat: 'Client Acquisition',
    bullets: [
      'AI qualifies and responds to inbound leads 24/7',
      'Automated multi-channel follow-up sequences',
      'Integrates with your existing CRM in days',
      'Clients report 3× faster response time to new leads'
    ]
  },
  {
    front: 'Answer Every Call. Close More Deals.',
    cat: 'Voice Infrastructure',
    bullets: [
      'AI voice agents handle calls, FAQs, and bookings',
      'Natural-sounding, trained on your business tone',
      'Escalates to human staff when needed',
      'Works after hours, on weekends, during peak seasons'
    ]
  },
  {
    front: 'Eliminate the Work That Clogs Your Week',
    cat: 'Workflow Automation',
    bullets: [
      'End-to-end automation of repetitive internal processes',
      'Connects your tools without a developer on retainer',
      'Custom-built on n8n, Make, or native APIs',
      'Average time saved: 18 hours/week per client'
    ]
  },
  {
    front: 'Your CRM Should Work, Not Just Exist',
    cat: 'CRM Management',
    bullets: [
      'Auto-update contacts, deals, and tasks from any trigger',
      'AI-driven lead scoring and pipeline management',
      'Sync across email, calls, forms, and messaging apps',
      'Works with HighLevel, HubSpot, Salesforce, and more'
    ]
  },
  {
    front: 'Support That Never Sleeps or Burns Out',
    cat: 'Customer Support',
    bullets: [
      'AI handles Tier-1 support across chat, email, and phone',
      'Trained on your knowledge base and FAQs',
      'Escalation paths built in for complex issues',
      'Reduce support load by up to 70% from week one'
    ]
  },
  {
    front: 'Kill Manual Data Entry, Permanently',
    cat: 'Document Processing',
    bullets: [
      'Extract, validate, and route data from any document',
      'Works with invoices, contracts, forms, and PDFs',
      'Integrates into your existing approval workflows',
      'Eliminates transcription errors at the source'
    ]
  },
];
          ...
          <div class="form-step" id="step-2">
            ...
            <div class="form-group">
              ...
              <div class="pill-group" role="radiogroup" aria-label="Biggest bottleneck">
                ${['Lead follow-up', 'Internal operations', 'Customer support', 'Document handling'].map((opt, i) => `
                  <div class="pill-radio">
                    <input type="radio" id="bn-${i}" name="bottleneck" value="${opt}">
                    <label for="bn-${i}">${opt}</label>
                  </div>
                `).join('')}
              </div>
            </div>
...
const faqs = [
  {
    ...
    a: 'For most clients, the first automation is running within 7–14 days of the audit. Complex multi-system builds take 3–6 weeks. We\'ll tell you exactly during the free audit — no vague timelines.'
  },
  {
    q: 'Do we need to change the tools we already use?',
    a: 'Almost never. We build around your existing stack. If a tool is creating the problem, we\'ll tell you, but we don\'t push replacements you don\'t need.'
  },
  {
    q: 'What happens if something breaks after handover?',
    a: 'Every build includes a 30-day post-launch support window. Ongoing support retainers are available for businesses that want Oshlink to maintain and expand their systems over time.'
  },
  {
    q: 'Is this just a chatbot?',
    a: 'No. Chatbots are one tool in a much larger automation ecosystem. We build full workflows: data pipelines, voice agents, CRM automation, document processing, multi-step triggers — whatever your operation actually needs.'
  },
  {
    q: 'How much does it cost?',
    a: 'Engagements vary by scope. The free audit gives you a clear project scope and a fixed-price quote before you commit to anything. Most clients recoup the investment within the first 60–90 days.'
  }
];
    ...
    <div class="faq-container reveal">
      ${faqs.map((f, i) => `
        <div class="faq-item" id="faq-item-${i}">
          ...
          <div class="faq-answer" id="faq-ans-${i}" role="region" aria-labelledby="faq-btn-${i}">
...
// ============================================================
function updateROI() {
  const team = parseInt(document.getElementById('s-team').value);
  const hours = parseInt(document.getElementById('s-hours').value);
  const rate = parseInt(document.getElementById('s-rate').value);

  document.getElementById('v-team').textContent = team;
  document.getElementById('v-hours').textContent = hours;
  document.getElementById('v-rate').textContent = `$${rate}`;

  const annualSpend = team * hours * 52 * rate;
  const savings = Math.round(annualSpend * 0.6);
  const paybackMonths = Math.max(1, Math.round(15000 / (savings / 12)));

  const fmt = n => '$' + n.toLocaleString('en-AU');
  document.getElementById('roi-spend').textContent = fmt(annualSpend);
  document.getElementById('roi-save').textContent = fmt(savings);

  let paybackText;
  if (paybackMonths <= 1) paybackText = '~1 month';
  else if (paybackMonths <= 2) paybackText = '~2 months';
  else if (paybackMonths <= 3) paybackText = '~3 months';
  else paybackText = `~${paybackMonths} months`;
  document.getElementById('roi-payback').textContent = paybackText;
}

['s-team', 's-hours', 's-rate'].forEach(id => {
...
step1Next?.addEventListener('click', () => {
  ...
  let valid = true;
  [firstName, email, industry].forEach(el => {
  ...
  dot2.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
});

document.getElementById('audit-form-el')?.addEventListener('submit', (e) => {
  e.preventDefault();
